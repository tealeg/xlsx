...
// rowLimit is the maximum number of rows to read. If the value is -1, there is no limit.
func readRowsFromSheet(Worksheet *xlsxWorksheet, file *File, sheet *Sheet, rowLimit int) ([]*Row, []*Col, int, int) {
	...
	// If there is no Dimension tag, an invalid Dimension tag, or if there is a limit on the number of rows,
	// infer the dimension from the sheets that we read.
...
// readZipReaderInternal is the maximum number of rows to read. If the value is -1, there is no limit.
